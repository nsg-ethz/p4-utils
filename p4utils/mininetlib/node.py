...
class P4Switch(Switch):
    ...
    def stop(self, deleteIntfs=True):
        ...
        if self.switch_running():
            ...
            if not wait_condition(self.switch_running, False, timeout=SWITCH_STOP_TIMEOUT):
...
class Tofino(Switch):
    ...
    def __init__(self, name,
        ...
        if not os.path.isdir(os.path.join(self.log_dir, self.name)):
            if os.path.exists(os.path.join(self.log_dir, self.name)):
                raise NotADirectoryError("'{}' exists and is not a directory.".format(os.path.join(self.log_dir,self.name)))
            else:
    ...
    def start(self, controllers=None):
        ...
        debug('P4 switch {} PID is {}.\n'.format(self.name, self.switch_pid))
        if not wait_condition(self.bin_running, True, timeout=SWITCH_START_TIMEOUT):
            raise ChildProcessError('Tofino switch {} did not start correctly. Check the switch log file.'.format(self.name))
        ...
        if not wait_condition(self.driver_running, True, timeout=SWITCH_START_TIMEOUT):
